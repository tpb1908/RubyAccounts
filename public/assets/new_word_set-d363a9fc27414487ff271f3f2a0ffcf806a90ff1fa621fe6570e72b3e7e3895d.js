...
function analyse() {
	...
	words = input.value.replace(/\s+/g, " ").split(' ');
	deepAnalysis(words);
	word_count = input.value == "" ? 0 : words.length;
...
function deepAnalysis(words) {
	...
	values.forEach(function(v) {
		...
		out += v.word + " : " + v.count + " (" + word_keys + "), ";
		word_keys = 0;
	});
	average_keys = Math.round((keypress_count/words.length) * 100)/100;
	$('#words').text(out);
}
